...
(defparameter *endpt* (foreign-alloc '(:pointer pjsip-endpoint)))
(defparameter *cp* (foreign-alloc 'pj-caching-pool))
(defparameter *med-endpt* (foreign-alloc '(:pointer pjmedia-endpt)))
...
;;;Call variables
(defvar *inv* '(foreign-alloc '(:pointer pjsip-inv-session)))
(defvar *med-stream* (foreign-alloc '(:pointer pjmedia-stream)))
